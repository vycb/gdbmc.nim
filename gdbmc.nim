...
const
  ...
  LIBGDBM = "libgdbm.so" #"/usr/lib/x86_64-linux-gnu/libgdbm.so"
  GDBMH = "<gdbm.h>"
  #}}}
type
  DATUM* = object #{{{
  ...
  GDBM_EXCEPTION* = object of Exception
...
proc Exists*(db: Database, key: string): bool =
  ...
  let
    kcs = cstring(key)
    datk = DATUM(dptr: kcs, dsize: kcs.len.cint)
...
proc update*(db:Database, key :string, value :string, flag: GDBM_STORE_FLAG): (GDBM_STORE_RESULT, GDBM_ERRORS) =
  ...
  let
    kcs = cstring(key)
    vcs = cstring(value)
    datumk = DATUM(dptr: kcs, dsize: kcs.len().int32())
    datumv = DATUM(dptr: vcs, dsize: vcs.len().int32())
...
proc Fetch*(db: Database, key: string): string =
  ...
  let
    kcs = cstring(key)
    datk = DATUM(dptr: kcs, dsize: kcs.len.cint) #mk_datum(kcs)
